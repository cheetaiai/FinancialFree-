...
export const PWAInstallButton: React.FC<PWAInstallButtonProps> = ({
  ...
  return (
    <>
      <button
        onClick={() => setIsModalOpen(true)}
        className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl border border-emerald-500/30 bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-700 dark:text-emerald-400 text-xs font-bold transition-all cursor-pointer select-none ${className}`}
        title="Download Android APK (.apk), Install PWA, or Preview Mobile & Tablet Modes"
      >
        <Smartphone size={14} className="text-emerald-600 dark:text-emerald-400 flex-shrink-0" />
