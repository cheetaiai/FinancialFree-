...
export const api = {
  // Auth
  login: (email: string, password: string, hcaptchaToken?: string) =>
    request<{ token: string; user: User }>('/api/auth/login', {
      method: 'POST',
      body: JSON.stringify({ email, password, hcaptchaToken })
    }),

  register: (data: { email: string; password: string; name?: string; phone?: string; hcaptchaToken?: string }) =>
  ...
  forgotPassword: (email: string, hcaptchaToken?: string) =>
    request<{ success: boolean; message: string; code?: string }>('/api/auth/forgot-password', {
      method: 'POST',
  ...
  sendVerificationCode: () =>
    request<{ success: boolean; message: string; code?: string }>('/api/auth/send-verification-code', {
      method: 'POST'
    }),
