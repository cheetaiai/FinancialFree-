...
import { generateFinancialInsights, draftReminderMessage, chatFinancialAssistant, scanReceiptOrImage, analyzeGraphTrends } from './server/gemini.ts';
...
app.get('/api/database/status', requireAuth, (req: Request, res: Response) => {
  res.json({
    ...
    databaseId: 'ai-studio-financialfree-3a870ab5-7f77-495e-adb9-543690ead6fd',
    ...db.getStatus()
  });
});
...
app.get('/api/backup/export', requireAuth, (req: Request, res: Response) => {
  try {
    ...
    res.setHeader('Content-Disposition', `attachment; filename="FinancialFree_Backup_${new Date().toISOString().split('T')[0]}.json"`);
    res.json(backup);
  } catch (error: any) {
...
app.get('/api/export/csv', requireAuth, (req: Request, res: Response) => {
  try {
    ...
    if (type === 'people') {
      ...
      let csv = 'Person ID,Full Name,Phone,Email,Category,Total Given (INR),Total Returned (INR),Remaining Balance (INR),Status,Last Transaction\n';
      ...
      res.setHeader('Content-Disposition', `attachment; filename="FinancialFree_People_${new Date().toISOString().split('T')[0]}.csv"`);
      return res.send(csv);
    } else {
      const txs = db.getTransactions({});
      let csv = 'Transaction ID,Date,Person Name,Type,Amount (INR),Payment Method,Month,Year,Financial Year,Purpose,Notes\n';
      for (const t of txs) {
      ...
      res.setHeader('Content-Type', 'text/csv');
      res.setHeader('Content-Disposition', `attachment; filename="FinancialFree_Transactions_${new Date().toISOString().split('T')[0]}.csv"`);
      return res.send(csv);
    }
  } catch (error: any) {
